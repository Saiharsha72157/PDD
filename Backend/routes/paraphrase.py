...
async def paraphrase_text(request: Request, data: ParaphraseRequest, current_user: dict = Depends(get_optional_current_user)):
    ...
    try:
        ...
        if (language != "English" or has_indic) and groq_clients:
...
"hallucinationDetected": true/false
            ...
            result = (parsed.get("paraphrasedText") or result_raw).strip()
...
def get_paraphrase_history(current_user: dict = Depends(get_current_user)):
    ...
    try:
        return history_manager.get_history(user_id=str(current_user.get("user_id", "")))
    except Exception as e:
...
def get_paraphrase_favorites(current_user: dict = Depends(get_current_user)):
    """Retrieves all starred favorite rephrasing records."""
    try:
        return history_manager.get_favorites(user_id=str(current_user.get("user_id", "")))
    except Exception as e:
        print(f"[Backend] Error fetching favorites: {e}")
        raise HTTPException(status_code=503, detail=f"Failed to fetch favorites: {str(e)}")

@router.post("/paraphrase/favorite/{entry_id}")
def toggle_paraphrase_favorite(entry_id: str, current_user: dict = Depends(get_current_user)):
    """Toggles the favorite state of a history entry."""
    try:
        # Check ownership
        hist = history_manager.get_history()
        target = next((x for x in hist if x.get("id") == entry_id), None)
        if not target or target.get("user_id") != current_user.get("user_id"):
        ...
        raise HTTPException(status_code=503, detail=f"Failed to toggle favorite: {str(e)}")

@router.delete("/paraphrase/history/{entry_id}")
def delete_paraphrase_history(entry_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail=f"Failed to delete history: {str(e)}")
...
def analyze_writing_text(data: WritingAnalysisRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail=f"Writing analysis failed: {str(e)}")

    ...
    def search(self, query: str, limit: int = 20, page: int = 1) -> List[Dict[str, Any]]:
        ...
        if clean_query in ["", "ai", "machine learning", "ml", "all"]:
            ...
            results = [
                ...
                if clean_query in str(ds.get("title", "")).lower() 
                or clean_query in str(ds.get("description", "")).lower()
            ]
...
class DatasetService:
    ...
    def search_all(self, query: str, provider: str = "kaggle", page: int = 1, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        if provider_key == "all":
            ...
            kaggle_results = []
            try:
                kaggle_prov = self.providers.get("kaggle")
                if kaggle_prov:
                    kaggle_results = kaggle_prov.search(query, limit=limit, page=page)
            except Exception as e:
                print(f"[DatasetService] Kaggle search failed in 'all': {e}")
            
            uci_results = []
            try:
                uci_prov = self.providers.get("uci")
                if uci_prov:
                    uci_results = uci_prov.search(query, limit=limit, page=page)
            except Exception as e:
